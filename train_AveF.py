...
from models.DN_net_AveF import DN_Net, Discriminator
...
def evaluate(g1,dataset,device,filename):
    ...
    with torch.no_grad():
        ...
        noise1, noise2,reconstruct_gt=g1.test(img.to(device),noisy_sub1.to(device),noisy_sub2.to(device))
        grid_rec=make_grid(un_normalize(reconstruct_gt.to(torch.device("cpu"))),nrow=3)
        # print(grid_rec.shape)
        noise_ave=noise1.to(torch.device("cpu"))
        reconstruct_gt = reconstruct_gt.to(torch.device("cpu"))
        noise_no=noise2.to(torch.device("cpu"))
...
def train_model(g1,d1,dataloader,val_dataset,num_epochs,parser,save_model_name="model"):
    ...
    for epoch in range(start,num_epochs+1):
        ...
        for images,gt in tqdm(dataloader):
            ...
            #获取生成器生成的图片
            noise1, noise2,reconstruct_gt=g1(images,noisy_sub1,noisy_sub2)
            ...
            g_l_data2 = criterion_mse(noise1, mask)  # 噪声分布图像损失
            g_l_data3 = criterion_mse(noise2, mask)  # 减噪噪图像损失
