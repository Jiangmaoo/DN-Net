...
def plot_log(data, save_model_name="model"):
    ...
    # plt.ylabel("loss")
    # plt.title("Loss")
    plt.savefig("./logs1/"+ save_model_name +".png")
    ...
    for epoch in range(start,num_epochs+1):
        ...
        for images,gt in tqdm(dataloader):
            ...
            g_loss=lambda_dict["lambda1"]*g_l_data1+lambda_dict["lambda2"]*g_l_c_gan1+\
                lambda_dict["lambda1"]*g_l_data2
                   # +lambda_dict["lambda1"]*g_l_data3
            g_loss.backward()
            optimizer_g.step()
            # print(g_loss)
        ...
        #采用间隔几个epoch保存模型
        if epoch%10==0:
            torch.save(g1.state_dict(),"checkpoints1/"+save_model_name+"_G1_"+str(epoch)+".pth")
            torch.save(d1.state_dict(),"checkpoints1/"+save_model_name+"_D1_"+str(epoch)+".pth")

            g1.eval()
            evaluate(g1,val_dataset,device,"{:s}/val_{:d}".format("result1",epoch))
...
def train(parser):
    ...
    if parser.load is not None:
        ...
        g1.load_state_dict(fix_model_state_dict(torch.load("./checkpoints1/S-R-Net_G1_"+parser.load+'.pth')))
        d1.load_state_dict(fix_model_state_dict(torch.load("./checkpoints1/S-R-Net_D1_"+parser.load+'.pth')))
